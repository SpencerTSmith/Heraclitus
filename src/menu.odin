...
update_menu :: proc()
{
  ...
  if key_repeated(.DOWN) || key_repeated(.S) { advance_item(+1) }
  if key_repeated(.UP)   || key_repeated(.W) { advance_item(-1) }

  if mouse_scrolled_up()   { advance_item(-1) }
  if mouse_scrolled_down() { advance_item(+1) }
